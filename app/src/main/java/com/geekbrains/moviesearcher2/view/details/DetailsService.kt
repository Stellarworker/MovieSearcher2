...
class DetailsService(name: String = "DetailService") : IntentService(name) {
    ...
    override fun onHandleIntent(intent: Intent?) {
        intent?.let {
            when (val id = intent.getIntExtra(ID_EXTRA, -1)) {
                -1 -> onEmptyData()
                else -> loadDetails(id)
            }
        } ?: run {
    ...
    private fun onResponse(details: MovieDetails) {
        details.title?.let {
    ...
    private fun onSuccessResponse(details: MovieDetails) {
        putLoadResult(DETAILS_RESPONSE_SUCCESS_EXTRA)
        broadcastIntent.putExtra(DETAILS_EXTRA, details)
